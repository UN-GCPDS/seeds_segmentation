...
def get_encoder(input_shape=[None, None, 3], trainable=True, name="encoder"):
    """
    Creates an encoder based on MobileNetV2.
    Extracts features at different resolutions for use in the skip connections.
    """
    # Ensure input dimensions are compatible with MobileNetV2 (multiple of 32)
...
def fcn_mobilenetv2(input_shape=(128, 128, 3), out_channels=1, out_ActFunction='sigmoid', trainable = False, name="fcnMobileNetV2"):
    """
    Creates a Fully Convolutional Network based on MobileNetV2 for semantic segmentation.
    
    Args:
        input_shape: Input image shape, should be divisible by 32 for optimal performance
        num_classes: Number of output classes (1 for binary segmentation)
        trainable: Whether to fine-tune the MobileNetV2 backbone
        name: Model name
        
    Returns:
        A Keras Model for semantic segmentation
    """
    # Verify input dimensions are compatible with MobileNetV2
